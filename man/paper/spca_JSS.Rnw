...
\SweaveOpts{engine = R, pdf = TRUE, eps = FALSE, keep.source = TRUE}
...
<<r qq-plot, fig = TRUE, message = FALSE, warning = FALSE,  width = 3, height= 3>>=
...
<<r barplot, fig = TRUE, message = FALSE, warning = FALSE,  width = 5, height= 5>>=
plot(ho_spcadef)
@
A more compact circular plot is produced by selecting \code{plot_type = "c"}

<<r circplot, fig = TRUE, message = FALSE, warning = FALSE,  width = 5, height= 5>>=
...
<<r heatmap_compare, fig = TRUE, message = FALSE, warning = FALSE,  width = 5, height= 5>>=
...
We fit a pSPCA thet produce sPCs that have at least 0.95 correlation with the PCs, using pSPCA and backward elimination to select the variables.
<<r pspca>>=
ho_pspca = spca(holzinger, ncomps = 4, alpha = 0.95,  method = "p",
                stop_criterion = "r2", var_selection = "b")
summary(ho_pspca)
@
Numerical and visual comparisons with `ho_spcadef` can be obtained calling \fct{compare_spca}
<<r compare, fig = true, >>=
compare_spca(list(ho_spcadef, ho_pspca), plot_loadings = TRUE)
@

with \code{alpha = 0.95}, objective \code{"r2}
stop_criterion
