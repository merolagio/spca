...
options(prompt = "R> ", continue = "+  ", width = 70, useFancyQuotes = FALSE, prefix.string = "spca_JSS")
#library("MASS")
@
...
We first establish notation. Let $X$ denote the $n \times p$ data matrix, containing $n$ cases on $p$ features centered to mean zero, and let $S \propto X\trasp X$ denote either the covariance or correlation matrix. In what follows, we use the term covariance matrix for both cases, since the methods and software treat them in the same way.

The eigenvectors of $S$, $v_1, \ldots, v_p$ ($\vert\vert v_j \vert\vert = 1$, $j = 1, \ldots, p$), are the PC loadings and are ordered by decreasing corresponding eigenvalues,
...
Table \ref{tab:funcs_list} lists the functions available in the \pkg{spca} UI.
...
As a general rule, to inspect and compare LSSPCA solutions, we favor percentage contributions, that is loadings scaled to unit $L_1$ norm (sum of absolute values), over the usual loadings scaled to unit $L_2$ norm (sum of squared values). This is because the contributions are more easily interpreted. Loadings are used when \code{contribution = TRUE}, which is the default.

\subsection[Data]{\fct{Data}}
<<load_all, echo = FALSE, results = hide>>=
...
dim(holzinger)
@
\subsection[Preliminary pca()]{Preliminary \fct{pca}}
We usually begin the analysis by inspecting PCA results. This is useful to understand how effective PCA is for the dataset and to decide how many components to retain. 

The function \fct{pca} takes the following arguments
\begin{Code}
pca(M, ncomps = NULL, center_data = FALSE, scaledata = FALSE, 
    screeplot = TRUE, qq_plot = TRUE, nrow_data = NULL, neigen_toplot = NULL), 
\end{Code}
\code{M} is either a datamatrix or a covariance matrix, scaledat
...
Calling \fct{pca} with the default (`ncomps = NULL`) produces all PCA components with all components.
<<pca, fig = FALSE, message = FALSE, warning = FALSE>>=
ho_pca = pca(holzinger, screeplot = TRUE, qq_plot = FALSE)
@
\begin{figure}[H]
\centering
<<pca_scree, fig = TRUE, echo = FALSE, message = FALSE, warning = FALSE,  width = 5, height= 5, include = FALSE>>=
pca(holzinger, screeplot = TRUE, qq_plot = TRUE)
@
<<pca_wac, fig = TRUE, echo = FALSE, message = FALSE, warning = FALSE,  width = 5, height= 5, include = FALSE>>=
pca(holzinger, screeplot = TRUE, qq_plot = TRUE)
@
...
\begin{figure}[H]
\centering
<<qq-plot, fig = TRUE, echo = TRUE, message = FALSE, warning = FALSE,  width = 5, height= 5, include = FALSE>>=
...
\begin{figure}[H]
\centering
<<barplot, fig = TRUE, message = FALSE, warning = FALSE,  width = 5, height= 5, include = FALSE>>=
plot(ho_spcadef)
@
...
\begin{figure}[H]
\centering
<<circplot, fig = TRUE, message = FALSE, warning = FALSE,  width = 5, height= 5, include = FALSE>>=
