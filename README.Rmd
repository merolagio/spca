...
```
## Usage
The  main function *spca()* computes the sparse loadings and various statistics, such as the variance explained by each sparse component (sPC). In a typical LS-SPCA workflow, the number of sPCs to compute is chosen by examining visually the eigenvalues of the covariance matrix. These can be computed and stored in a compatible object of class *spca*  with the function  *pca()*.
...
Utilities available are *compare_spca()* (to compare two or more spca solutions) and *new.spca()* (to create an *spca* object from a set of loadings) are available.
