...
// 
// @noRd
double makeCvexpOneCompSC_int(const Eigen::Ref<const Eigen::MatrixXd>& A, const Eigen::Ref<const Eigen::MatrixXd> &S,  
                              double &cvexp){
  Eigen::MatrixXd B = A.transpose() * S;
  Eigen::MatrixXd M = (B * B.transpose()) * (B * A).inverse() ; 
  cvexp = M.trace();
  return(cvexp);
}
...
void uspcaC(
    const Eigen::Ref<const Eigen::MatrixXd>& S, 
           ...
           int comp_number, 
           Eigen::VectorXi& indj, 
           int cardt, //indj size p, cardt card current solution
           Eigen::VectorXi& indjm1, //indjm1 size p
           int cardm1, //cardm1 total card previous solution 
           Eigen::MatrixXd& R, 
           Eigen::VectorXd& a, 
           bool& singular,
           bool PM, // false, 
  ...
  try {
    if (comp_number > 0){
    Sp = Sd.inverse();
    // R constraints = A_[comp_number-i, p] S = A[p,1:(comp_number-1)]' S
    // computes one row each past component
    // R = A' S so (comp_number - 1) x p
    for(int i = 0; i < p; i++){
      st = 0;
    ...
    // R' M^-1 K
    // different for second component because M^-1 is scalar    
    if( comp_number == 1){
      // Q = R'/M(0,0)
      ...
      //  Q.topLeftCorner(cardt, comp_number) * K.topLeftCorner(comp_number, cardt);
    } //end comp_number == 1   
    else{
      // Mp = M^-1
      Mp = M.inverse();
      ...
      for(int i = 0; i < cardt; i++){
        for(int ii = 0; ii < comp_number; ii++){
          st = 0;
          for(int iii = 0; iii < comp_number; iii++){
            st = st + R(iii, indj(i)) * Mp(iii, ii);
          }
          Q(i, ii) = st;
        }
      }
    ...
    // computed C, now computes P = C D[cardt, cardt] * C'
    
    //  P = C *SS[ind, ind] *C'
      maked_loopColF(S, D, indt);
      P.topLeftCorner(cardt, cardt) =  C * D.topLeftCorner(cardt, cardt) * C.transpose();
