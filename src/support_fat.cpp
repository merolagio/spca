                          ...
                          int maxiterPM)
{
  ...
  for (int j = 0; j < ncomps; ++j) {
    ...
    if (!std::isfinite(val) || val <= 0.0)
      Rcpp::stop("PMnEigenpairs: non-positive eigenvalue in fat backend");
...
void deflT_F(Eigen::MatrixXd PC, Eigen::MatrixXd& K, double curvexp, double& vexp) {
  try {
    ...
    if (PC.rows() != n)
      Rcpp::stop("deflT_F: incompatible dimensions of PC and K");
    if (!PC.allFinite() || !K.allFinite())
      Rcpp::stop("deflT_F: PC and K must contain only finite values");
    
    Eigen::MatrixXd G = PC.transpose() * PC;
    Eigen::LDLT<Eigen::MatrixXd> ldltG(G);
    if (ldltG.info() != Eigen::Success || !ldltG.isPositive())
      Rcpp::stop("deflT_F: PC' PC is singular or not positive definite");
    
    Eigen::MatrixXd Pt = PC.transpose();
    Eigen::MatrixXd GP = ldltG.solve(Pt);
    if (!GP.allFinite())
      Rcpp::stop("deflT_F: solve failed; PC' PC is singular");
    
    Eigen::MatrixXd P = Eigen::MatrixXd::Identity(n, n) - PC * GP;
    
    K = P * K * P;
    vexp = curvexp - K.trace();
  }
  catch (std::exception& ex) {
    Rcpp::stop("deflT_F failed: " + std::string(ex.what()));
  }
  catch (...) {
    Rcpp::stop("deflT_F failed: unknown C++ error");
  }
}
...
void cspcaTC(const Eigen::Ref<const Eigen::MatrixXd>& X,
  ...
  try {
    ...
    if (PM) {
      vec = GeigvecPMC(P, Sd, eigval, epsPM, maxiterPM);
    } else {
      Eigen::GeneralizedSelfAdjointEigenSolver<Eigen::MatrixXd> es(P, Sd);
      if (es.info() != Eigen::Success)
        throw std::runtime_error("generalized eigen solver failed in cspcaTC");
    ...
    if (!std::isfinite(eigval) || !std::isfinite(nv) || nv <= 0.0)
      throw std::runtime_error("invalid generalized eigen solution in cspcaTC");
    ...
    scores.col(comp_number).setZero();
  }
}
  ...
  try {
    ...
    if (comp_number > 0) {
      ...
      if (ldltSd.info() != Eigen::Success || !ldltSd.isPositive())
        throw std::runtime_error("selected crossproduct submatrix is singular in uspcaTC");
      
      Eigen::MatrixXd R = Xd.transpose() * scores.leftCols(comp_number);
      Eigen::MatrixXd K = ldltSd.solve(R);
      if (!K.allFinite())
        throw std::runtime_error("solve with selected crossproduct submatrix failed in uspcaTC");
      
      Eigen::MatrixXd M = R.transpose() * K;
      
      if (comp_number == 1) {
        if (!std::isfinite(M(0, 0)) || std::abs(M(0, 0)) <= 0.0)
          throw std::runtime_error("constraint denominator is zero in uspcaTC");
        ...
        if (ldltM.info() != Eigen::Success)
          throw std::runtime_error("constraint matrix factorization failed in uspcaTC");
        
        Eigen::MatrixXd RK = ldltM.solve(K.transpose());
        if (!RK.allFinite())
          throw std::runtime_error("constraint matrix solve failed in uspcaTC");
        
        C = Eigen::MatrixXd::Identity(cardt, cardt) - R * RK;
      }
    ...
    if (PM) {
      ...
      Eigen::GeneralizedSelfAdjointEigenSolver<Eigen::MatrixXd> es(P, Sd);
      if (es.info() != Eigen::Success)
        throw std::runtime_error("generalized eigen solver failed in uspcaTC");
      vec = es.eigenvectors().col(cardt - 1);
      eigval = es.eigenvalues()(cardt - 1);
    }
    ...
    const double nv = vec.norm();
    if (!std::isfinite(eigval) || !std::isfinite(nv) || nv <= 0.0)
      throw std::runtime_error("invalid generalized eigen solution in uspcaTC");
...
void pspcaTC(
  ...
  try {
    ...
    Eigen::LDLT<Eigen::MatrixXd> ldltSd(Sd);
    if (ldltSd.info() != Eigen::Success || !ldltSd.isPositive())
      throw std::runtime_error("selected crossproduct submatrix is singular in pspcaTC");
    
    Eigen::VectorXd rhs = Xd.transpose() * r;
    Eigen::VectorXd vec = ldltSd.solve(rhs);
    if (!vec.allFinite())
      throw std::runtime_error("solve with selected crossproduct submatrix failed in pspcaTC");
    
    const double nv = vec.norm();
    if (!std::isfinite(nv) || nv <= 0.0)
      throw std::runtime_error("invalid solution in pspcaTC");
