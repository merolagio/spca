...
{  ### ===========================================================================
   ## v14.4 added comments changed thresvar and threshvaronPC to be vexp not losses
   ## v14.3.3 added msg = TRUE for choosecard
   ## v14.3.3 added PRCvexp to tracing sperem ben
   ## v14.3.1 added tracing should be removed?
   ## added choosecard, returns only tracing
   ## added tracevarloss
   ## v14.1 renamed loadings
   ## v14 July 2014 reiinstated makezM  
   ## April 10 fixed the problem of too few indices if excludeload or startind
   ## Nov 16, 2013 fixed some messages
   ### v13 included excludeload so that parsimonious loadings can be computed
   ### v12 is like v11.2 with few improvements
   ### v11.2 fixed naming for only 1 component
   ### v11.1  checked automatic setting of mincard and thresh for thresvaronPC setting, should be alright july 29
   ## fixed problem with Z for correlated comps use makezM now but seems same same
   ##v11 added treshvaronPC, trimming is stopped if proportion of total var explained 
   #### less than PC's
   ## v10 added reduced trimming for last laodings fixed problem with only one mincard
   ## v9 added unc as vector so that some of the comps can be computed correlated
   #### fixed problem with only 1 index, made option ignore threshvar
   ## v8 added ndbyvexp to determine dimension with vexpPC
   ## v7 added mincard and output and convergence (0 thresh, 1 mincard, 2 maxvloss)
   ## v6 changed the stopping rule for reached min cardinality
   ## v5 added perc, oprion for trimming percentage loadings
   ### v4 added trimming multiple coeff and improved control on variance loss
   ...
   ####   if nd missing or ndbyvexp not = 1, nd is chosen or can be made smaller by this rule
   ## thresh is vector of thresholds. if scalar all equal to that
   ####   if thresh shorter than nd missing ones set to last thresh
   ##  ndbyvexp is object variance total explained, 
   ####  if < 1 computation is stopped to that number of components 
   ## unc if TRUE uncorrelated components else correlated
   #####  if a vector is given, some comp can be correlated and some not    
   ## startind is list of indices from which to choose non zero loadings
   ####    if NULL trimming starts from all indices
   ## trim = 1, number of coefficients to eliminate at each iterations.
   ####    will leave at least j coefficients if uncorrelated required
   ## threshvaronPC nd-vector or 1 value,controls the loss of cumulated variance on PCA's for each component. 
   ####    if = FALSE, no control, if less doesn't allow more than the given loss for each component
   ####     it takes priority over threshvar, if both specified
   ## threshvar nd-vector or 1 value,controls the loss of variance for each component. 
   ####    if = FALSE, no control, if less doesn't allow more than the given loss for each component
   ## 
   ## eps value below which the abs value of loading is considered zero
   
   ### ++++++++++ OUTPUT
   ## A matrix of loadinfs
   ## vexpPC variance explained by principal components
   ## vexpv variance explained by sparse components
   ## vexpo variance explained by sparse components not trimmed
   ## startind list of indices of non zero loadings
   ## totvcloss loss of cumulated variance explained by solutions from principal components
   ## vossbe loss of variance explained by solutions from non trimmed components
   ## niter, number of loadings trimmed for each components
   ## converged, type of convergence: 0 threshold, 1 min cardinality reached, 2 maxvarloss reached,
   ## thresh, threshold for trimming used
   ## threshvar, threshold for variance loss used
   ## ndbyvexp, False if set to 1 or value used
   ## unc, which components are uncorrelated
   ## covComp, present only if any unc = FALSE
   ## output is class spca (S3)  
}
### ===========================================================================  
  p = ncol(S)
...
if (length(threshvaronPC) == 1)
  ...
  if (length(indlast[[j]]) < j & changedunc[j] == TRUE)
    converged[j] = 3
  # ---- ccheck if CVEXP reached ndbyvexp   -------------------------
