...
spcabb = function(S, card, unc = TRUE, startind, excludeload = FALSE, nvexp = FALSE, 
                  msg = TRUE){
 { ## v4.2.1 re changed sorting using uspca loadings June 14
   ##     changed sorting and back sorting of indices using rank, works for incomplete startind
   ## v4.1 allowed incomplete startind and added temporary tmp.A output
   ## v4
   ## included excludeload
  ## fixed makezM for uncorr
  ## added startind, it allows to search subset of indices
  # ---- superseeds all spcabb, allows mixed cor and uncor and vexp as in paper
  ## new: allows mixed corr and uncorr components
  ## unc can be vector, if only one made vector
  ## excludeload: vector (length nd or shorter) of logical for excluding load already included
   ## nvexp = TRUE uses the real vexp, otherwise that maximised by cls-spca
  ## can use a' Sj Sj a/(a' Sj a) for vexp. the real extra sum of squares
  ## runs B&B for spca for all cardinalities
  ##
 } 
 tryCatch({
  p = ncol(S)
...
#        vv = apply((S[])^2,1,sum)/ diag(S)
        vv = abs(uspca(S, ind = nind)$a)
        vv = vv[nind]
        #        if order(decreasing = TRUE) use         ra = (lnind + 1) - rank(vv)
       # ra = rank(vv)
        ra = (lnind + 1) - rank(vv)
        ...
        niter = tmp$niter
        #indo = nind[inds[1:length(nind)]][tmp$w]
...
branchSpca = function(D, k, A = NULL, Z = NULL, ind = FALSE, mod = c("uncorr","corr", "ncorr")) {
  ...
  ## called by U-SPCA-BB and C-SPCA-BB
  #D covariance matrix
  # k caedinality
  # A previous loadings
