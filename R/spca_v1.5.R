...
#' @param fixed_index_list \code{NULL} or list of numeric/integer vectors.
#'   Elements may also be \code{NULL}. Indices cannot contain missing values.
...
validate_spca_inputs = function(M,
  ...
  if (!is.null(ncomp_by_cvexp)) {
    if (!is.numeric(ncomp_by_cvexp) || length(ncomp_by_cvexp) != 1 ||
        ...
        (ncomp_by_cvexp > 1))
      stop("ncomp_by_cvexp must be NULL or a numeric scalar in (0, 1]", call. = FALSE)
  }
...
if (!is.null(method)) {
  ...
  if (!is.null(intensive) && !is.boolean(intensive))
    stop("intensive must be NULL, TRUE, or FALSE", call. = FALSE)
  
  if (!is.null(fat_matrix) && !is.boolean(fat_matrix))
    stop("fat_matrix must be NULL, TRUE, or FALSE", call. = FALSE)
  
  if (!is.null(fixed_index_list)) {
    if (!is.list(fixed_index_list))
      stop("fixed_index_list must be NULL or a list", call. = FALSE)
    
    ok_list = vapply(fixed_index_list,
                     function(x) is.null(x) || is_int(x),
                     logical(1))
    if (!all(ok_list))
      stop("each element of fixed_index_list must be NULL or an integer-valued numeric vector", call. = FALSE)
    
    has_na = vapply(fixed_index_list,
  ...
  if (!is.null(center_data) && !is.boolean(center_data))
    stop("center_data must be NULL, TRUE, or FALSE", call. = FALSE)
  
  if (!is.null(scale_data) && !is.boolean(scale_data))
    stop("scale_data must be NULL, TRUE, or FALSE", call. = FALSE)
  
  if (!is.null(pm_loading) && !is.boolean(pm_loading))
  ...
  if (!is.null(maxiter_pm_loading)) {
    if (!is_int(maxiter_pm_loading) || length(maxiter_pm_loading) != 1 || (maxiter_pm_loading < 1))
      stop("maxiter_pm_loading must be NULL or a positive integer scalar", call. = FALSE)
  }
  
  if (!is.null(pm_varsel) && !is.boolean(pm_varsel))
...
#'   supplied.
#' @param method A character vector selecting the LS-SPCA variant. Values may
#'   be `"cspca"`, `"uspca"`, or `"pspca"`; only the first letter is used. If
#'   fewer values than components are supplied, the last value is reused for the
...
#'   backend is used.
#' @param fixed_index_list A list of integer-valued vectors giving fixed
#'   1-based variable indices for selected components. Use `NULL` or
#'   `integer(0)` for components without fixed indices.
...
spca = function(M,
                ...
                pm_loading = FALSE,
                eps_pm_loading = 1e-5,
                maxiter_pm_loading = 100,
                pm_varsel = FALSE,
                eps_pm_varsel = 1e-5,
                maxiter_pm_varsel = 200) {
  ...
  if (use_fat_backend) {
    ...
    if (var_selection != "fwd") {
      stop("Only forward selection is implemented for  fat matrices", 
           call. = FALSE)
    }
  }
...
#  browser()
  ...
  if (length(fixed_index_list) > 0) {
    if (ncomps_cpp == 0)
      stop("when fixed_index_list is used, n_comps must be positive or ncomp_by_cvexp must be in (0, 1)", call. = FALSE)
