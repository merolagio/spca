<!--
Adapted from JSS artticle to an HTML vignette.
...
fits are still loaded, not recomputed; only the cheap Holzinger plots run.
-->
---
...
We report percentage *contributions* (weights scaled to unit $l_1$ norm) rather than $l_2$-scaled weights, as they are easier to interpret. The `print()` and `plot()` methods use contributions by default; set `contributions = FALSE` for weights.
...
pca(M, n_comps = NULL, center_data = FALSE, scale_data = FALSE,
    fat_matrix = NULL, screeplot = FALSE, qq_plot = TRUE, n_obs = NULL,
...
`pca()` can draw a screeplot and a Wachter qq-plot (Wachter, 1976), which compares the eigenvalues with the Marchenko–Pastur quantiles (Winkler 2021). The qq-plot applies to covariance matrices of equal-variance variables; for correlation matrices set `common_var = 1`, and supply `n_obs` when `M` is a covariance matrix. The standalone `scree_plot()` and `mp_qqplot()` functions customize these plots. The plots below show the screeplot and the qq-plot, the latter with a line fitted to the smallest $(n - 3)$ eigenvalues.
...
mp_qqplot(ho_pca, n_vars = ncol(holzinger),
               n_fitline = -3)
```
...
Passing PC weights to `pc_weights` overlays them for comparison (not for circular plots); a vector or factor in `variable_groups` colours the plot by group. The default is a bar plot.
...
plot(ho_spcadef, n_plot = 3, plot_type = "c", controls = list(color_scale = "printsafe", variable_names = "auto"))
```
...
`fixed_index_list` fixes the variable subset of selected sPCs. Its elements correspond to components in order; a `NULL` or empty element requests variable selection for that component. A list of length one fixes only the first component, whereas fixing a later component requires placeholders for the preceding components. Index sets may overlap and need not include every variable. If the list is longer than `n_comps`, the extra elements are ignored with a warning. The `holzinger` variables fall into four scales (`holzinger_scales`); we fit a four-component solution with each sPC weight on a single scale. Contributions and summaries are in the two tables below.
...
*Contributions with each sPC weight on a single scale.*
...
Except for the first, the pSPCA weights have larger cardinality and explain more variance, as VEXP cannot fall below the optimised $R^2$. The first three weight sets are broadly similar across methods, and those sPCs are highly correlated; the fourth differs, again reflecting mostly noise.

### Variable groups

Questionnaire variables often belong to scales; the 12 `holzinger` variables form four. `aggregate_by_group()` sums the weights or contributions of an `spca` object by the groups in `variable_groups`.

*Contributions aggregated by scale.*

```{r aggr}
aggregate_by_group(ho_spcadef, variable_groups = holzinger_scales)
```
...
`new_spca()` brings solutions from other analyses into the *spca* framework. It needs a set of weights and either the covariance or the data matrix.
```{r new_spca}
