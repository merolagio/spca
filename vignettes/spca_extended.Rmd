...
Computations run in the **C++** backend, which uses referenced arrays to avoid deep copies. Fat matrices are handled with a reverse-SVD in the row space. The covariance matrix $S = X\trasp X$ and the product $SS$ are formed once, and deflations use rank-1 updates rather than recomputation, reducing the per-step cost from about $O(p^3)$ to $O(p^2)$. The leading eigen-pair can optionally be computed with the power method (`pm_weight = TRUE` for weights, `pm_varsel = TRUE` for selection), controlled by the `maxiter_pm...` and `eps_pm...` arguments.
...
spca(M, alpha = 0.95, n_comps = NULL, ncomp_by_cvexp = NULL,
    ...
    pm_weight = FALSE, eps_pm_weight = 1e-05,
    maxiter_pm_weight = 100, pm_varsel = FALSE,
